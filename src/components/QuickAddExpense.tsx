...
export default function QuickAddExpense() {
  ...
  const [payer, setPayer] = useState<Payer>('common')
  ...
  const handleSubmit = () => {
    ...
    addExpense({
      name: note.trim() || t(selected.labelKey),
  ...
  const payerOptions: PayerOption[] = [
    ...
    ...(household?.personBName
      ? [{ value: PAYER.PersonB, label: household.personBName, color: household?.personBColor }]
      : []),
  ]

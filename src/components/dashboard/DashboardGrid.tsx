...
import type { Layouts, LayoutItem } from '../../types'
...
interface DashboardGridProps {
  ...
  flowWidgetIds?: string[]
}

// flowWidgetIds: widget IDs rendered outside the grid with natural height on desktop
export default function DashboardGrid({ widgets, flowWidgetIds = [] }: DashboardGridProps) {
  const { t } = useTranslation()
  ...
  const resizeHandle = useMemo(() => {
    ...
    return (axis: string, ref: React.Ref<HTMLElement>) => (
      <div
  ...
  const filteredLayouts = useMemo(() => {
    ...
    for (const [bp, items] of Object.entries(filtered)) {
      ...
      const missing = visibleIds
        ...
        .map((id) => defaults.find((d) => d.i === id))
        .filter((d): d is LayoutItem => d !== undefined)
      if (missing.length > 0) {
        filtered[bp] = [...items, ...missing]
      }
    }
    return filtered
  }, [layouts, visibleIds])
  ...
  const handleLayoutChange = useCallback(
    (_layout: Layout, allLayouts: ResponsiveLayouts<string>) => {
      if (!isEditMode) return
      // ResponsiveLayouts is Partial — merge with current layouts,
      // keeping only defined breakpoints from the callback.
      const merged: Layouts = { ...layouts }
      for (const [bp, items] of Object.entries(allLayouts)) {
        if (items) merged[bp] = items
      }
      setLayouts(merged)
    },
    [setLayouts, isEditMode, layouts]
  )

  // Mobile: reorder with up/down buttons in edit mode
  const moveWidget = useCallback((id: string, direction: number) => {
    const smLayout = layouts.sm || DEFAULT_LAYOUTS.sm
            ...
            <div className="flex items-center justify-between px-3 py-1.5 bg-brand/5">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  aria-label={t('dashboardGrid.moveUp')}
                >
                ...
                <button
                  ...
                  className="p-1 rounded text-text-muted hover:text-white disabled:opacity-20 transition-colors"
                  aria-label={t('dashboardGrid.moveDown')}
                >

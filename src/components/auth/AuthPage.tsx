...
export default function AuthPage({ inviteCode, defaultMode }: AuthPageProps) {
  const { t } = useTranslation()
  const translateError = useErrorTranslation()

  const [mode, setMode] = useState<'login' | 'signup'>(inviteCode ? 'signup' : (defaultMode || 'login'))
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      // No need to setGoogleLoading(false) on success — page redirects to Google
    } catch {
      setError(t('auth.unexpectedError'))
      setGoogleLoading(false)
    }
        ...
        >
          <div className="text-center mb-8">
            <div className="flex items-center justify-center gap-2 mb-3">
              <img src="/logo-crown-sm.webp" alt="Monest" className="w-10 h-10" />
      ...
      >
        ...
        <Card className="p-6">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white text-gray-800 font-medium text-sm rounded-xl hover:bg-gray-100 transition-colors disabled:opacity-50"
          >
            {googleLoading ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (

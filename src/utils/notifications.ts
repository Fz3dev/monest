 ...
 */
function getDailyResteAVivre(): number | null {
  ...
  try {
    ...
    if ('periodicSync' in registration) {
      ...
      if (status.state === 'granted') {
        await (registration as unknown as { periodicSync: { register: (tag: string, options: { minInterval: number }) => Promise<void> } }).periodicSync.register('weekly-reminder', {
          minInterval: WEEK_MS,
        })
        return true
      }
 ...
 */
export function checkAndShowWeeklyNotification(): void {

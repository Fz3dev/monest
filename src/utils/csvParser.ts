...
import type { DetectedColumns, RecurringCharge, Frequency } from '../types'
      ...
      complete: (results: ParseResult) => {
        ...
        if (sample.includes('Ã©') || sample.includes('Ã¨') || sample.includes('Ã ')) {
          ...
          Papa.parse(file, {
            ...
            skipEmptyLines: true,
            encoding: 'latin1',
            complete: (r: ParseResult) => resolve(r),
...
function detectFrequency(dates: (string | null)[]): Frequency {
  if (dates.length < 2) return 'monthly'
  const sorted = [...dates].sort()
  const diffs: number[] = []
  for (let i = 1; i < sorted.length; i++) {
    const d1 = new Date(sorted[i - 1] as string)
    const d2 = new Date(sorted[i] as string)
...
export function detectRecurring(data: Record<string, string>[], columns: DetectedColumns): RecurringCharge[] {
  ...
  return Object.entries(groups)
    ...
    .map(([key, txs]) => {
      ...
      return {
        ...
        dates: txs.map((t) => t.date).filter((d): d is string => d !== null).sort(),
      }
    })

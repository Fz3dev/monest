...
import type { FixedCharge, InstallmentPayment, PlannedExpense, Household, MonthlyEntry } from '../types'

// ─── isActiveThisMonth ───────────────────────────────────────

describe('isActiveThisMonth', () => {
  it('returns false if charge is inactive', () => {
    expect(isActiveThisMonth({ active: false } as FixedCharge, '2026-03')).toBe(false)
  })

  it('returns true if charge is active with no date bounds', () => {
    expect(isActiveThisMonth({ active: true } as FixedCharge, '2026-03')).toBe(true)
  })

  it('returns false if month is before startsAt', () => {
    expect(isActiveThisMonth({ active: true, startsAt: '2026-06' } as FixedCharge, '2026-03')).toBe(false)
  })

  it('returns true if month equals startsAt', () => {
    expect(isActiveThisMonth({ active: true, startsAt: '2026-03' } as FixedCharge, '2026-03')).toBe(true)
  })

  it('returns false if month is after endsAt', () => {
    expect(isActiveThisMonth({ active: true, endsAt: '2026-01' } as FixedCharge, '2026-03')).toBe(false)
  })

  it('returns true if month equals endsAt', () => {
    expect(isActiveThisMonth({ active: true, endsAt: '2026-03' } as FixedCharge, '2026-03')).toBe(true)
  })

  it('returns true if month is within startsAt and endsAt', () => {
    expect(isActiveThisMonth({ active: true, startsAt: '2026-01', endsAt: '2026-12' } as FixedCharge, '2026-06')).toBe(true)
  })
})
...
describe('isDueThisMonth', () => {
  const monthly = { active: true, frequency: 'monthly' } as FixedCharge
  const bimonthly = { active: true, frequency: 'bimonthly', startMonth: 1 } as FixedCharge
  const bimonthlyEven = { active: true, frequency: 'bimonthly', startMonth: 2 } as FixedCharge
  const quarterly = { active: true, frequency: 'quarterly', startMonth: 1 } as FixedCharge
  const quarterlyFeb = { active: true, frequency: 'quarterly', startMonth: 2 } as FixedCharge
  const annual = { active: true, frequency: 'annual', startMonth: 3 } as FixedCharge
  ...
  it('returns false for inactive charge regardless of frequency', () => {
    expect(isDueThisMonth({ active: false, frequency: 'monthly' } as FixedCharge, '2026-03')).toBe(false)
  })
})
...
describe('getEffectiveMonth', () => {
  it('returns same month when no delay', () => {
    expect(getEffectiveMonth({ paymentDelayMonths: 0 } as FixedCharge, '2026-03')).toBe('2026-03')
  })

  it('returns previous month with delay of 1', () => {
    expect(getEffectiveMonth({ paymentDelayMonths: 1 } as FixedCharge, '2026-03')).toBe('2026-02')
  })

  it('handles year boundary', () => {
    expect(getEffectiveMonth({ paymentDelayMonths: 1 } as FixedCharge, '2026-01')).toBe('2025-12')
  })

  it('handles delay of 2', () => {
    expect(getEffectiveMonth({ paymentDelayMonths: 2 } as FixedCharge, '2026-03')).toBe('2026-01')
  })
})
...
describe('getInstallmentNumber', () => {
  const installment = { firstPaymentDate: '2026-01-15', installmentCount: 4 } as InstallmentPayment
...
describe('computeMonth', () => {
  ...
  it('computes basic income minus charges', () => {
    const charges = [
      { id: '1', name: 'Loyer', amount: 1000, payer: 'common', frequency: 'monthly', active: true } as FixedCharge,
    ]
    const entry = { incomeA: 3000, incomeB: 2000 } as MonthlyEntry
  ...
  it('handles personal charges correctly', () => {
    const charges = [
      { id: '1', name: 'Loyer', amount: 1000, payer: 'common', frequency: 'monthly', active: true } as FixedCharge,
      { id: '2', name: 'Tel Alice', amount: 30, payer: 'person_a', frequency: 'monthly', active: true } as FixedCharge,
      { id: '3', name: 'Tel Bob', amount: 25, payer: 'person_b', frequency: 'monthly', active: true } as FixedCharge,
    ]
    const entry = { incomeA: 3000, incomeB: 2000 } as MonthlyEntry
    const result = computeMonth('2026-03', household, charges, [], [], entry)
  ...
  it('applies pro rata split correctly', () => {
    const prorataHousehold = { ...household, splitMode: 'prorata' } as Household
    const charges = [
      { id: '1', name: 'Loyer', amount: 1000, payer: 'common', frequency: 'monthly', active: true } as FixedCharge,
    ]
  ...
  it('pro rata falls back to splitRatio when no income', () => {
    const prorataHousehold = { ...household, splitMode: 'prorata', splitRatio: 0.6 } as Household
    const charges = [
      { id: '1', name: 'Loyer', amount: 1000, payer: 'common', frequency: 'monthly', active: true } as FixedCharge,
    ]
  ...
  it('handles custom split ratio', () => {
    const customHousehold = { ...household, splitMode: 'custom', splitRatio: 0.7 } as Household
    const charges = [
      { id: '1', name: 'Loyer', amount: 1000, payer: 'common', frequency: 'monthly', active: true } as FixedCharge,
    ]
    const entry = { incomeA: 3000, incomeB: 2000 } as MonthlyEntry
  ...
  it('includes planned expenses for the month', () => {
    const planned = [
      { id: 'p1', name: 'Vacances', estimatedAmount: 2000, payer: 'common', targetMonth: '2026-03' } as PlannedExpense,
    ]
    const entry = { incomeA: 3000, incomeB: 2000 } as MonthlyEntry
  ...
  it('excludes planned expenses for other months', () => {
    const planned = [
      { id: 'p1', name: 'Vacances', estimatedAmount: 2000, payer: 'common', targetMonth: '2026-08' } as PlannedExpense,
    ]
  ...
  it('applies variable overrides to fixed charges', () => {
    const charges = [
      { id: 'c1', name: 'Electricite', amount: 100, payer: 'common', frequency: 'monthly', active: true } as FixedCharge,
    ]
    const entry = { incomeA: 3000, incomeB: 2000, variableOverrides: { c1: 150 } } as unknown as MonthlyEntry
    const result = computeMonth('2026-03', household, charges, [], [], entry)
  ...
  it('handles solo mode with only person A', () => {
    const charges = [
      { id: '1', name: 'Loyer', amount: 800, payer: 'person_a', frequency: 'monthly', active: true } as FixedCharge,
    ]
    const entry = { incomeA: 3000 } as MonthlyEntry
  ...
  it('skips inactive charges', () => {
    const charges = [
      { id: '1', name: 'Loyer', amount: 1000, payer: 'common', frequency: 'monthly', active: false } as FixedCharge,
    ]
    const entry = { incomeA: 3000, incomeB: 2000 } as MonthlyEntry
    const result = computeMonth('2026-03', household, charges, [], [], entry)
  ...
  it('deducts negative starting balance from reste a vivre', () => {
    ...
    const entry = { incomeA: 3000, incomeB: 2000, startingBalanceA: -200, startingBalanceB: -100 } as MonthlyEntry
    const result = computeMonth('2026-03', household, charges, [], [], entry)
  ...
  it('handles positive starting balance', () => {
    ...
    const entry = { incomeA: 3000, incomeB: 2000, startingBalanceA: 500, startingBalanceB: 0 } as MonthlyEntry
    const result = computeMonth('2026-03', household, charges, [], [], entry)
  ...
  it('respects bimonthly frequency', () => {
    const charges = [
      { id: '1', name: 'Assurance', amount: 200, payer: 'common', frequency: 'bimonthly', startMonth: 1, active: true } as FixedCharge,
    ]
    const entry = { incomeA: 3000, incomeB: 2000 } as MonthlyEntry

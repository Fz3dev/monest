...
describe('detectColumns', () => {
  it('detects BoursoBank columns', () => {
    const headers = ['Date opération', 'Libellé', 'Débit', 'Crédit']
  ...
  it('detects English columns', () => {
    const headers = ['Date', 'Label', 'Debit', 'Credit']
    const result = detectColumns(headers)
  ...
  it('handles columns with extra spaces', () => {
    const headers = ['  Date  ', ' Libellé ', ' Débit ']
    const result = detectColumns(headers)
    expect(result.dateCol).toBe('  Date  ')
    expect(result.labelCol).toBe(' Libellé ')
    expect(result.debitCol).toBe(' Débit ')
...
describe('detectRecurring', () => {
  ...
  it('returns empty array when no debitCol', () => {
    const noDebitCols = { ...columns, debitCol: null }
    const data = [{ date: '01/01/2026', label: 'TEST', debit: '100,00' }]

...
interface LangContent {
  nav: { features: string; login: string; cta: string }
  hero: {
    badge: string
    title: string
  ...
  langSwitch: { label: string; path: string }
}
...
const CONTENT: Record<string, LangContent> = {
  fr: {
    ...
    hero: {
      ...
      subtitle: 'Un outil de budget moderne pour gérer vos finances en solo ou à deux. Suivez vos dépenses, répartissez les charges en couple si vous le souhaitez, et construisez de vraies habitudes d\'épargne.',
    ...
    features: {
      ...
      items: [
        ...
        { title: 'Solo ou en couple', description: 'Utilisez Monest seul ou à deux. En mode partagé, répartissez les charges librement — égal, pourcentage ou au prorata des revenus.' },
  ...
  en: {
    ...
    hero: {
      ...
      subtitle: 'A modern budget tracker for managing your finances solo or as a couple. Track spending, split charges with your partner if you want, and build real savings habits.',
      cta: 'Start for free',
    ...
    features: {
      ...
      items: [
        ...
        { title: 'Solo or together', description: 'Use Monest on your own or as a couple. In shared mode, split charges flexibly — equal, percentage, or income-based.' },
...
function PieChart({ items, size = 100 }: { items: CategoryItem[]; size?: number }) {
  const r = size / 2 - 4
  const cx = size / 2
  const cy = size / 2
  const offsets = items.reduce((acc: number[], item: CategoryItem, i: number) => {
    acc[i + 1] = acc[i] + item.pct / 100
    return acc
  }, [0])
  const paths = items.map((item: CategoryItem, i: number) => {
...
function DashboardMockup({ t }: { t: LangContent }) {
  const m = t.mockup
  return (
      ...
      <div className="relative bg-[#0F0F15] rounded-[2.5rem] border border-white/[0.1] p-3 shadow-2xl shadow-brand/10">
        ...
        <div className="bg-bg-primary rounded-[2rem] overflow-hidden">
          ...
          <div className="px-4 pb-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 grid grid-cols-2 gap-x-3 gap-y-1.5">
                {m.categoryItems.map((cat: CategoryItem) => (
          ...
          <div className="border-t border-white/[0.06] px-4 py-2.5 flex justify-around">
            {m.nav.map((label: string, i: number) => {
...
export default function LandingPage({ lang = 'fr' }: { lang?: string }) {
      ...
      <section className="py-20 sm:py-28 px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} variants={stagger} className="max-w-3xl mx-auto text-center">
          ...
          <motion.div variants={stagger} className="grid sm:grid-cols-3 gap-4">
            {t.problem.pains.map((pain: string, i: number) => {
      ...
      <section id="features" className="py-20 sm:py-28 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {t.features.items.map((feature: FeatureItem, i: number) => {
      ...
      <section id="how-it-works" className="py-20 sm:py-28 px-6 bg-white/[0.01]">
        <div className="max-w-4xl mx-auto">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger} className="grid sm:grid-cols-3 gap-6 sm:gap-8">
            {t.howItWorks.steps.map((item: StepItem, i: number) => {
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} variants={stagger} className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <motion.div variants={fadeUp} transition={{ duration: 0.5 }}>
              ...
              <div className="space-y-4">
                {t.security.items.map((text: string, i: number) => {

...
function CircularProgress({ percentage, size = 160, strokeWidth = 10 }: CircularProgressProps) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={`url(#${gradientId})`} strokeWidth={strokeWidth} strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} filter={`url(#${glowId})`} style={{ transition: 'stroke-dashoffset 1.2s cubic-bezier(0.4, 0, 0.2, 1)' }} />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <AnimatedNumber value={Math.round(percentage)} format={(v: number) => `${Math.round(v)}%`} className="text-3xl font-bold bg-gradient-to-r from-brand to-success bg-clip-text text-transparent" />
...
function EmojiSelector({ value, onChange, t }: EmojiSelectorProps) {
  return (
    <div>
...
function ColorPicker({ value, onChange, t }: ColorPickerProps) {
  return (
    <div>
        ...
        {GOAL_COLORS.map((color) => (
          <motion.button
            ...
            style={{
              ...
              ...(value === color ? { ['--tw-ring-color' as string]: color } : {}),
            }}
          />
        ))}
      </div>
...
interface GoalFormProps {
  initialValues?: SavingsGoal | null
  onSubmit: (data: GoalFormData) => void
  onCancel: () => void
  t: TFunction
}

function GoalForm({ initialValues, onSubmit, onCancel, t }: GoalFormProps) {
  const [form, setForm] = useState(
    initialValues
      ? { name: initialValues.name, targetAmount: String(initialValues.targetAmount), icon: initialValues.icon || '\u{1F4B0}', color: initialValues.color || GOAL_COLORS[0], deadline: initialValues.deadline || '' }
  ...
  return (
    <div className="space-y-4">
      <Input label={t('savings.goalName')} value={form.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('name', e.target.value)} placeholder={t('savings.goalNamePlaceholder')} />
      <Input label={t('savings.targetAmount')} type="number" value={form.targetAmount} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('targetAmount', e.target.value)} placeholder="0" suffix="€" />
      <EmojiSelector value={form.icon} onChange={(v: string) => update('icon', v)} t={t} />
      <ColorPicker value={form.color} onChange={(v: string) => update('color', v)} t={t} />
      <Input label={t('savings.deadline')} type="month" value={form.deadline} onChange={(e: React.ChangeEvent<HTMLInputElement>) => update('deadline', e.target.value)} />
...
function ContributeForm({ goal, onSubmit, onCancel, t }: ContributeFormProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Input label={t('savings.amountToAdd')} type="number" value={amount} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAmount(e.target.value)} placeholder="0" suffix="€" />
...
function GoalCard({ goal, index, onEdit, onContribute, onDelete, t }: GoalCardProps) {
...
function EmptyState({ onAdd, t }: EmptyStateProps) {
  return (
...
export default function SavingsPage() {
  ...
  const handleAddGoal = useCallback((data: GoalFormData) => {
    const { deadline, ...rest } = data
    addGoal({ ...rest, currentAmount: 0, deadline: deadline || undefined })
    toast.success(t('savings.goalCreated', { name: data.name }))
    setModal(null)
  }, [addGoal, t])

  const handleEditGoal = useCallback((data: GoalFormData) => {
    if (!modal || modal.type !== 'edit') return
    const { deadline, ...rest } = data
    updateGoal(modal.goalId, { ...rest, deadline: deadline || undefined })
    toast.success(t('savings.goalEdited'))
    setModal(null)
  }, [modal, updateGoal, t])

  const handleContribute = useCallback((amount: number) => {
    if (!modal || modal.type !== 'contribute' || amount <= 0) return
